          ...
          <q-card-section>
            <div class="row items-start justify-between q-mb-xs">
              <div>
                ...
                <!-- Optional: Show monthly payment info if they entered it -->
                <q-chip v-if="loan.monthly_payment > 0" size="sm" color="red-2" text-color="red-10" class="text-weight-bold q-ml-none q-mt-xs">
                  ₱{{ formatMoney(loan.monthly_payment) }} / mo ({{ loan.term_months }} mos)
                </q-chip>
              </div>
            ...
            <div class="row q-gutter-sm q-mb-md bg-white q-pa-sm rounded-borders shadow-1 q-mt-sm">
              ...
              <div class="col text-center">
                <div class="text-caption text-red-5 text-weight-bold">TOTAL PAYABLE</div>
            ...
            <div class="row q-mt-md">
              <q-btn class="full-width" color="red-9" icon="payment" label="Make Payment" @click="openPaymentDialog(loan)" unelevated />
            </div>
          </q-card-section>
        </q-card>
      </div>
      ...
      <q-card style="min-width: 350px; width: 100%; max-width: 500px; border-radius: 12px;">
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit="submitNewLoan" class="q-gutter-md">
            ...
            <q-input v-model.number="newLoanData.principal_amount" type="number" step="0.01" label="Original Price / Principal (₱)" outlined dense required />
            
            <div class="q-mt-md">
              <div class="text-caption text-grey-8 q-mb-xs">How do you want to calculate the cost?</div>
              <q-btn-toggle
            ...
            <!-- Dynamic Inputs based on the toggle -->
            <div class="row q-col-gutter-sm q-mt-sm">
              <template v-if="newLoanData.calc_mode === 'interest'">
                <div class="col-12">
                  <q-input v-model.number="newLoanData.interest_amount" type="number" step="0.01" label="Total Interest Charged (₱)" outlined dense required />
                </div>
              </template>

              <template v-if="newLoanData.calc_mode === 'total'">
                <div class="col-12">
                  <q-input v-model.number="newLoanData.total_payable" type="number" step="0.01" label="Total Amount to be Paid (₱)" outlined dense required />
                </div>
              </template>

              <template v-if="newLoanData.calc_mode === 'monthly'">
                <div class="col-6">
                  <q-input v-model.number="newLoanData.monthly_payment" type="number" step="0.01" label="Monthly Payment (₱)" outlined dense required />
                </div>
                <div class="col-6">
                  <q-input v-model.number="newLoanData.term_months" type="number" label="Term (Months)" outlined dense required />
                </div>
              </template>
            </div>
            ...
            <q-input v-model="newLoanData.date" type="date" label="Date of Loan" outlined dense required />

            <div class="row justify-end q-mt-md">
              <q-btn flat label="Cancel" color="grey" v-close-popup />
              <q-btn type="submit" label="Create Loan" color="red-9" :loading="financeStore.isLoading" unelevated />
            </div>
          </q-form>
        </q-card-section>
      </q-card>
      ...
      <q-card style="min-width: 350px; border-radius: 12px;">
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit="submitPayment" class="q-gutter-md">
            ...
            <q-select 
              v-model="paymentData.from_account" 
              :options="spendingAccounts" 
              option-label="name" 
              option-value="id"
              map-options
              emit-value
              label="Pay From (Spending Account)" 
              outlined 
              dense 
              required
            />
...
const newLoanData = reactive({
  ...
  calc_mode: 'monthly', // Default to monthly since it's the most common!
...
// --- Smart Derivation Engine for the Form ---
...
const computedTotal = computed(() => {
  return (Number(newLoanData.principal_amount) || 0) + computedInterest.value
})


// --- Refactored Math Engine ---
const getTotalBurden = (loan) => (Number(loan.principal_amount) || 0) + (Number(loan.interest_amount) || 0)

const getAmountPaid = (loan) => {
  return financeStore.transactions
    .filter(t => t.status === 'cleared' && t.loan_id === loan.id)
...
// --- Actions ---
const openAddLoanDialog = () => { addLoanDialog.value = true }
const resetNewLoanForm = () => {
  newLoanData.name = ''; newLoanData.principal_amount = null; newLoanData.calc_mode = 'monthly';
...
const submitNewLoan = async () => {
  // We construct the payload using our smart computed properties!
  const payload = {
    name: newLoanData.name,
    principal_amount: Number(newLoanData.principal_amount) || 0,
    interest_amount: computedInterest.value, // Ground truth saved to DB
    monthly_payment: newLoanData.calc_mode === 'monthly' ? (Number(newLoanData.monthly_payment) || 0) : 0,
    term_months: newLoanData.calc_mode === 'monthly' ? (Number(newLoanData.term_months) || 0) : 0,
    date: newLoanData.date,
    is_active: true
  }

  const result = await financeStore.createLoan(payload)
  if (!result.error) {
...
const openPaymentDialog = (loan) => {
  ...
  // Pre-fill the exact monthly payment if they provided one!
...
const submitPayment = async () => {
  const payload = {
    ...
    loan_id: activeLoan.value.id, 
    description: paymentData.description || `Payment to ${activeLoan.value.name}`
  }
